...
**Minor** — In `client.ts`, `refreshDocument` closes and reopens a document with version always set to 1 (via `ensureOpen`). Some LSP servers track document versions across open/close cycles and may behave oddly when the version doesn't increment.
