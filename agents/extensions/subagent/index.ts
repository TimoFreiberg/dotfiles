	...
	pi.registerTool({
		...
		async execute(_toolCallId, params, signal, onUpdate, ctx) {
			...
			if (!params.tasks || params.tasks.length === 0) {
				...
				return {
					content: [{ type: "text", text: `No tasks provided. Available agents: ${text}` }],
					details: { results: [] },
				};
			}
			...
			const emitParallelUpdate = () => {
				if (onUpdate) {
					...
					onUpdate({
						content: [
							{ type: "text", text: `${done}/${allResults.length} done, ${running} running...` },
						],
			...
			if (results.length === 1) {
				...
				if (isError) {
					...
					return {
						content: [{ type: "text", text: `Agent ${r.stopReason || "failed"}: ${errorMsg}${outputFileHint(results)}` }],
						details: { results },
						isError: true,
					};
				}
				return {
					content: [{ type: "text", text: (getFinalOutput(r.messages) || "(no output)") + outputFileHint(results) }],
					details: { results },
				};
			}
		...
		renderCall(args, theme) {
			...
			if (tasks.length === 1) {
				const t = tasks[0];
				const preview = t.task ? (t.task.length > 60 ? `${t.task.slice(0, 60)}...` : t.task) : "...";
				let text = theme.fg("toolTitle", theme.bold("subagent ")) + theme.fg("accent", t.agent || "...");
