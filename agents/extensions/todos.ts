...
function tdo(args: string[]): string {
  const cmd = ["tdo", ...args].map(shellEscape).join(" ");
  try {
...
/** Read a todo's full content via `tdo view`. */
function readTodo(id: string): Todo | null {
  const output = tdo(["view", id]);
  if (!output) return null;

  const lines = output.split("\n");
  // First line: "<id>  <title>"
  const headerMatch = lines[0]?.match(/^([0-9a-f]{4})\s{2}(.*)$/i);
  ...
  // Parse metadata lines (status:, created:, assigned:)
  let bodyStart = 1;
  for (let i = 1; i < lines.length; i++) {
    ...
    if (metaMatch) {
      ...
      else if (key === "assigned") assigned = value;
      bodyStart = i + 1;
    } else {
      break;
    }
  ...
  return { id: todoId, title, status, assigned, created, body };
}
...
function splitTodos(todos: Todo[]) {
  ...
  for (const t of todos) {
    if (isDone(t.status)) closed.push(t);
...
function filterTodos(todos: Todo[], query: string): Todo[] {
  ...
  return matches
    .sort((a, b) => {
      const ac = isDone(a.todo.status),
        bc = isDone(b.todo.status);
...
function renderTodoHeading(theme: Theme, todo: Todo): string {
  const closed = isDone(todo.status);
...
function loadRefineInstructions(): { text: string; error?: string } {
  ...
  try {
    ...
    const section = (
      nextSection >= 0 ? after.slice(0, nextSection) : after
    ).trim();
    if (!section) {
...
function buildRefinePrompt(id: string, title: string): string {
  return REFINE_PROMPT_MARKER + "\n" + `refine task ${id} "${title}"\n`;
}
...
class TodoSelectorComponent extends Container implements Focusable {
  ...
  private updateHeader() {
    const openCount = this.allTodos.filter((t) => !isDone(t.status)).length;
    ...
    for (let i = startIdx; i < endIdx; i++) {
      if (i === 0) {
        ...
        this.listContainer.addChild(
          new Text(prefix + this.theme.fg(color, label), 0, 0),
        );
      } else {
...
class TodoActionMenuComponent extends Container {
  ...
  constructor(
    ...
    this.addChild(
      new Text(
        theme.fg("accent", theme.bold(`Actions for ${todo.id} "${title}"`)),
        0,
        0,
      ),
    );
    ...
    this.addChild(
      new Text(theme.fg("dim", "Enter to confirm • Esc back"), 0, 0),
    );
    this.addChild(new DynamicBorder((s: string) => theme.fg("accent", s)));
  }
...
class TodoDetailOverlayComponent {
  ...
  constructor(
    ...
    this.markdown = new Markdown(
      todo.body?.trim() || "_No details yet._",
      1,
      0,
      getMarkdownTheme(),
    );
  ...
  render(width: number): string[] {
    const maxHeight = Math.max(
      10,
      Math.floor((this.tui.terminal.rows || 24) * 0.8),
    );
    const headerLines = 3,
      footerLines = 3,
      borderLines = 2;
    ...
    const visibleLines = mdLines.slice(
      this.scrollOffset,
  ...
  private buildMetaLine(width: number): string {
    ...
    const statusColor = isDone(status) ? "dim" : "success";
    const assignText = this.todo.assigned
...
export default function todosExtension(pi: ExtensionAPI) {
  const todosCommandHandler = async (
    ...
    const todos = showDone
      ? allTodos.filter((t) => isDone(t.status))
      : allTodos.filter((t) => !isDone(t.status));
    ...
    await ctx.ui.custom<void>((tui, theme, _kb, done) => {
      ...
      const refreshList = () => {
        ...
        const filtered = showDone
          ? fresh.filter((t) => isDone(t.status))
          : fresh.filter((t) => !isDone(t.status));
      ...
      const copyPath = (todoId: string) => {
        ...
        const file = entries.find(
          (e) => e.startsWith(todoId) && e.endsWith(".md"),
        );
        if (!file) {
      ...
      const applyAction = async (
        ...
        refreshList();
        ctx.ui.notify(
          `${action === "done" ? "Done" : "Reopened"} todo ${todo.id}`,
          "info",
        );
        return "stay";
      };
      ...
      const showActionMenu = (todo: Todo) => {
        // Re-read to get full body for the menu context
        const record = readTodo(todo.id) ?? todo;
