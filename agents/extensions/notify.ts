...
export default function (pi: ExtensionAPI) {
  pi.on("agent_end", async (event, ctx) => {
    // OSC 777 only means anything when stdout is a real interactive terminal, i.e.
    // the "tui" run mode. `hasUI` is also true in "rpc" mode (e.g. the pilot web UI,
    // which embeds pi with a UI bridge) — gating on it there fired a stray escape
    // sequence into the host's stdout and a duplicate desktop notification. Guard on
    // the mode instead: only the terminal TUI should emit terminal escape codes.
