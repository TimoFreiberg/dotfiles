...
async function loadDefaultReviewGuidelines(): Promise<string> {
  try {
    const content = await fs.readFile(REVIEW_GUIDELINES_PATH, "utf8");
    const trimmed = content.trim();
    if (trimmed) return trimmed;
  } catch {
    // fall through
  }
  return "Review the code for correctness, security, performance, and maintainability. List findings with file paths, line numbers, and priority levels (P0-P3).";
}
