 ...
 *
 * Ported from https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/btw.ts
 *
 * Local adjustments:
 * - Dropped stripDynamicSystemPromptFooter: upstream regex targeted a
 *   "Current date and time:" footer that pi doesn't emit (pi uses
 *   "Current date: YYYY-MM-DD\nCurrent working directory: …"). The
 *   stripping isn't load-bearing for a side-chat running in the same
 *   cwd, so the side session now just uses ctx.getSystemPrompt() as-is.
 */
