...
const showFileSelector = async (
  ...
  const selection = await showFuzzyPicker(ctx, {
    title: "Select file",
    items,
    hint: "Type to filter • enter to select • ctrl+shift+d diff • esc to cancel",
    initialSelection: selectedPath ?? undefined,
    extraKeyHandler: (data, selectedItem) => {
      if (matchesKey(data, "ctrl+shift+d") && selectedItem) {
        const file = files.find(
          (entry) => entry.canonicalPath === selectedItem.value,
        );
        const canDiff =
